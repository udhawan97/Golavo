...
from .footballcsv import load_footballcsv_table, parse_footballcsv
...
def load_matches(pack_dir: Path) -> pd.DataFrame:
    ...
    elif source_id.startswith("footballcsv-"):
        frame = load_footballcsv_table(pack_dir)
    else:
