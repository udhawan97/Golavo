...
application receipts, checkpoints, archive/restore, and guarded calibration slices are
verified on `main` after that tag. They require current source, and Trust Center restore or
checkpoint creation specifically requires a desktop source build with its private launch token.
:::
