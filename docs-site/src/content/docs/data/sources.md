...
Football-Data.org, API-Football, Wikidata, weather sources, and ODbL overlays are not current dependencies. The Pappalardo/Wyscout CC-BY-4.0 research dataset is registered for a later isolated historical-research phase but is not installed yet. Future adoption requires the documented evidence and license gate; it is not implied by design docs. (OpenFootball has passed the relevant coverage audits and is vendored for historical seasons only — see [Coverage](/Golavo/data/coverage/).)
