...
| **2 — Source-mode web app** *(partly shipped)* | Matchday, Fixture Room, Forecast Theatre, After the Whistle; casual/expert; all data states. **Shipped:** Match Search over the full ~75k-match index and a per-match **Commentator's Notebook** computed at the pre-kickoff horizon. **Still deferred:** in-app (from-UI) sealing — the API stays read-only and seals are written by the engine CLI | performance & a11y budgets met; a live matchday sealed and scored | — |
