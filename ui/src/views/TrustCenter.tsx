...
export function TrustCenter() {
  ...
  const restore = async () => {
    if (!archiveFile || !preview) return;
    setArchiveError(null);
    let restored: ArchivePreview;
    try {
      restored = await restorePersonalArchive(archiveFile, replace);
    }
    catch (error) {
  ...
  return (
    <div className="stack" style={{ ["--gap" as string]: "1.25rem" }}>
      ...
      <section className="panel">
        ...
        <div className="panel__body stack">
          <p className="small dim">Forecasts, picks, and followed-match state only. Team favorites, credentials, provider settings, licensed overlays, weather, research captures, checkpoints, and caches are excluded.</p>
