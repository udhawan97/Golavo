...
import type { EvalSummary, ForecastArtifact } from "./contract";
import { loadMockEval, loadMockForecast, loadMockForecasts } from "../mocks";
...
function assertForecast(x: unknown, ctx: string): ForecastArtifact {
  ...
  if (a.schema_version !== SCHEMA_VERSION)
    throw new ContractError(`${ctx}: schema_version ${a.schema_version ?? "missing"} != ${SCHEMA_VERSION}`);
...
function assertEval(x: unknown, ctx: string): EvalSummary {
  ...
  if (e.schema_version !== SCHEMA_VERSION)
    throw new ContractError(`${ctx}: schema_version ${e.schema_version ?? "missing"} != ${SCHEMA_VERSION}`);
