...
- **75,079-match CC0 index** (verified by direct parquet read): 49,505 internationals (1872→2026-07-11, refreshed daily-ish via the martj42 pack) + 25,574 club matches across exactly the big-5 leagues (2010-14 season starts → May 2026). Goalscorers (47,888 rows, with minutes) and shootouts side-tables — internationals only. 100% CC0 sources; the ODbL overlay ships empty (`packs/README.md`, `overlay-odbl/`).
...
- **The ODbL overlay pack** (`packs/overlay-odbl/`) is a README describing OpenLigaDB Bundesliga detail — no data, no consuming code.
...
| OpenLigaDB | ODbL 1.0 — "Daten … unter der Open Database License" (openligadb.de) | results, goals, tables, goalgetters; near-live BL | Bundesliga 1/2/3, DFB-Pokal, **FIFA WC 2026**, CH Super League, Frauen-BL | current + live-ish | community, near-live | No | none published | Yes, share-alike | Yes | Yes, isolated | ODbL attribution + share-alike on derived DBs | none | community-entered | medium | **isolated share-alike pack** (overlay-odbl exists, empty) |
...
| GeoNames (dumps) | CC-BY ("commercial usage is allowed"; version unstated — minor UNVERIFIED) | places, coordinates | global | static | dumps | No (dumps) | 10k credits/day (API) | Yes + credit | Yes | Yes | link/credit to GeoNames | none | low | low | **bundled open core** (stadium geocoding, Later) |
...
5. **Bundesliga depth + WC 2026 detail**: OpenLigaDB via the existing (empty) ODbL overlay design — goalgetters/tables/near-live results — kept in a separate DB, never joined to the CC0 core (`scripts/check_license_isolation.sh` already enforces).
...
| **Open Core + share-alike pack** (opt-in) | + OpenLigaDB overlay (Bundesliga detail, WC26) | richer Bundesliga facts; ODbL notice shown |
