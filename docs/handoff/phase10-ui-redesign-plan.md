...
- **Settings + gear polish.** Replaced the lopsided (transform-offset) header gear with a clean centered one. Reworked the Settings page into `.settings__field` blocks so hint text sits with its control and wraps at a readable measure instead of bleeding full-width, and added a **Local intelligence** section that surfaces the AI-provider selector (Off / Local Ollama / Local llama.cpp / OpenAI or Anthropic BYOK) from Settings — off by default, local-first, honest copy (never changes a probability; shows unavailable in the sample build until a desktop engine is connected). `/settings` added to the overflow e2e coverage (now 24 overflow + 6 axe = 30 tests).
