...
| 🏟️ | **Open a domestic league's Season outlook** — title, top-four and relegation odds per model voice, with an optional one-fixture conditional result | Runs for all five bundled leagues after the complete-schedule gate. Conditional runs are explicit hypotheticals that are never saved, sealed, or fed back into a model |
...
| **Artifacts** | Versioned JSON contracts for forecasts, evidence bundles, facts, and AI narration; canonical payload hashes, source snapshot ids, and downloadable portable verification proofs |
