...
| **Distribution** | Unsigned macOS DMG and Windows MSI/EXE build paths plus checksums; signing/notarization/updater activate only when real secrets exist |
...
Outputs and `SHA256SUMS` land in `packaging/out/`. These builds are **unsigned**:
macOS requires right-click → **Open** on first launch; Windows requires
**More info → Run anyway**. That warning is the operating system accurately
describing the missing certificate, not Golavo asking you to lower your standards.

Signing, notarization, and signed updates are wired but gated on credentials the
project does not yet hold. Golavo would rather show an honest warning than cosplay
as a signed release. See [Installation](https://udhawan97.github.io/Golavo/installation/)
and [Updates & rollback](https://udhawan97.github.io/Golavo/updates-rollback/).
